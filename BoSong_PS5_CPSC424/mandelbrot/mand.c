//
// Parallel Mandelbrot with Dynamic Workload
//
...
int worker() {
  ...
  while (1) {
    ...
    if (status.MPI_TAG == 99) break; // Tag=99 means we're all done
    k = 0;
    ...
    MPI_Send(kbuf, bufsizei, MPI_INT, 0, task[0], MPI_COMM_WORLD); // sending color
  }
  timing(&time2, &cptime);
  total_time = time2 - time1;
  MPI_Barrier(MPI_COMM_WORLD);
